...
To answer the main question in a structured way, we use five focused research questions.

\begin{itemize}
  \item \textbf{RQ1:} Do VFT productivity and retrieval speed differ across semantic domains?
  \item \textbf{RQ2:} Which participant-level variables are associated with VFT outcomes?
  \item \textbf{RQ3:} Does SpAM spatial compactness differ across domains?
  \item \textbf{RQ4:} How strongly do participant SpAM clusters align with semantic and phonetic embedding-based clusters?
  \item \textbf{RQ5:} Does a combined VFT+SpAM fluency score improve inference compared to VFT-only scoring?
\end{itemize}
...
\label{tab:rq_simple}
\begin{tabular}{p{1cm} p{8cm} p{5.5cm}}
\toprule
...
\begin{tabular}{lrrrr}
\toprule
Domain & Session-domain N & Mean word count & Mean RT (ms) & Mean lexical diversity \\
\midrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{images/vft_wordcount_by_domain.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{images/vft_rt_by_domain.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{images/vft_words_vs_rt.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{images/spam_compactness_by_domain.png}
...
This negative correlation suggests that in this sample, larger total output is associated with lower self-reported confidence. This does not imply causality.

\textbf{Inference.}
For RQ2, there is evidence of monotonic association with selected participant variables, but direction should be interpreted carefully because confidence is self-reported and the sample is modest.
...
\end{itemize}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{images/spam_alignment_by_domain.png}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{images/cluster_similarity_matrix.png}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{images/sample_spam_semantic_phonetic_clusters.png}
...
\end{itemize}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{images/vft_vs_integrated_score.png}
  ...
  \begin{figure}[H]
  \centering
  \includegraphics[width=0.78\textwidth]{images/composite_score_distribution.png}
  ...
  \begin{tabular}{p{0.9cm} p{6.2cm} p{3.5cm} p{4.3cm}}
  \toprule
  RQ & Question & Main result & Inference \\
  \midrule
  RQ1 & Domain difference in VFT output/speed & Word count: $p=0.1515$; RT: $p=0.5894$ & No significant domain effect \\
  RQ2 & Participant variables and VFT outcomes & $\rho_{\text{words,conf}}=-0.395$ ($p=0.019$) & Significant monotonic association present \\
  RQ3 & Domain difference in SpAM compactness & Kruskal--Wallis $p=0.3857$ & No significant domain effect \\
  RQ4 & SpAM alignment with semantic/phonetic clusters & ARI sign tests significant; NMI around 0.36--0.41 & Positive directional evidence with moderate structural overlap \\
  RQ5 & Integrated score vs VFT-only score & $|\rho_{\text{integrated,conf}}| < |\rho_{\text{VFT,conf}}|$ and $p_{\text{integrated}}>0.05$ & Integrated score richer structurally, not stronger for this proxy \\
  \bottomrule
  \end{tabular}
  \end{table}
...
Second, SpAM and embedding clusters show moderate information overlap (NMI), and sign tests indicate positive ARI tendency in both semantic and phonetic channels. This indicates systematic structure linkage, but also leaves room for participant-specific strategies not captured by one global clustering rule.

Third, adding SpAM features gives a broader fluency profile, even though it did not increase correlation with confidence in this dataset. In simple words, integrated scoring gives richer structure, but not necessarily stronger external association for every target variable.
